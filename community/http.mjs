...
export function createHandler(service, readStars = async () => ({}), sources = null, documentation = null, translation = null, availability = null, extensions = null, github = null) {
  return async request => {
    try {
      ...
      if (url.pathname === `${path}/stars`) return request.method === 'GET' ? json(await readStars()) : json({ error: '请求方法不支持' }, 405, { allow: 'GET' });
      ...
      if (!sourceRoute && !translationRoute && !extensionRoute && !githubRoute && ![path, `${path}/read`].includes(url.pathname)) return json({ error: '接口不存在' }, 404);
      if (request.method === 'GET') return json(sourceRoute ? sources.status() : service.snapshot());
      ...
      if (sourceRoute) {
        ...
        return json(await sources.sync(command.sourceId));
      }
      let preparedFile;
