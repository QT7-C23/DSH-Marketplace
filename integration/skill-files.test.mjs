...
const moduleUrl = new URL('./plugin/resources/skill-files.mjs', import.meta.url);
const commit = 'a'.repeat(40);
const digest = bytes => createHash('sha256').update(bytes).digest('hex');
const blob = bytes => createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');
const license = readFileSync(new URL('../sources/licenses/openai-apache-2.0.txt', import.meta.url));
const anthropicLicense = readFileSync(new URL('../sources/licenses/anthropic-apache-2.0.txt', import.meta.url));
