...
import { credentialFile } from '../sources/github-credentials.mjs';
import { hostRuntime } from './plugin/host-runtime.mjs';
...
test('package is a closed installable bundle with runtime dependencies and actual bundled licenses', async () => {
  ...
  for (const name of ['LICENSE', 'DISCLAIMER.md', 'README.md', 'README.zh-CN.md', 'README.ja-JP.md', 'scripts/github-secret.ps1', 'sources/catalog.json', 'catalog/index.json', 'integration/plugin/client.js', 'BUNDLED_DEPENDENCIES.json']) assert(files.has('package/' + name), name);
  for (const name of files.keys()) assert(!/(?:^|\/)(?:artifacts|node_modules|tests|fixtures)\/|\.test\.|\.dpapi$|\.sqlite$|client\.tsx$/.test(name), name);
...
test('frontend build inputs remain inside the repository when invoked from outside it', () => {
  const script = `import { buildClient } from ${JSON.stringify(new URL('./build.mjs', import.meta.url).href)}; console.log(JSON.stringify((await buildClient()).inputs));`;
