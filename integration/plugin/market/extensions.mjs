/** @typedef {{name:string,version:string,route:string,state:string,removable:boolean,installed:boolean}} InstalledExtension */
...
/** @typedef {{id:string,action:'install'|'remove',name:string,version:string,profile:string,hostVersion:string,routes:string[],compatibility:{route:string|null,state:string,issues:string[]},allowed:boolean,license:string|null,source:string|null,permissions:Array<{name:string,scope:string}>,restartRequired:true}} ExtensionPlan */
/** @typedef {{status:'restart-required'|'failed'|'recovery-required',operationId:string,action:'install'|'remove',name:string,version?:string,backupCreated:boolean}} ExtensionResult */
...
export function parseInventory(value) {
  ...
  for (const row of value.items) ensure(string(row?.name) && string(row.version) && routes.includes(row.route) && states.includes(row.state) && typeof row.removable === 'boolean' && typeof row.installed === 'boolean');
  return value;
}

/** @returns {ExtensionPlan} */
export function parsePlan(value) {
  ensure(uuid(value?.id) && ['install', 'remove'].includes(value.action) && ['name', 'version', 'profile', 'hostVersion'].every(key => string(value[key])) && strings(value.routes) && value.routes.every(route => routes.includes(route)));
