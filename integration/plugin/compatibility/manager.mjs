...
export class ExtensionManager {
  ...
  async read() {
    ...
    const [native, standard] = await Promise.allSettled([this.native.read(), this.standard.read()]);
    ...
    return { schema: 1, profile: this.environment.profile, hostVersion: this.environment.hostVersion, adapter: standard.status === 'rejected' ? 'unknown' : this.standard.adapter() ? 'available' : 'missing', complete: native.status === 'fulfilled' && standard.status === 'fulfilled', items };
  }
  async prepare(command) {
    ...
    let candidate, compatibility, action;
    if (command.action === 'prepare-install') {
      ...
      action = 'install';
    } else if (command.action === 'prepare-remove') {
      ...
      if (dependents.length) throw new CommunityError(409, '其他已安装扩展仍依赖此包');
    ...
    const plan = { id, candidate, action, compatibility, allowed, fingerprint, expires: Date.now() + 10 * 60000 };
  ...
  async execute(command) {
    ...
    plan.promise = (async () => {
      ...
      const result = plan.action === 'install' ? await this.installer.install({ name, version, tarball, integrity }, plan.fingerprint) : await this.installer.remove(name, plan.fingerprint);
