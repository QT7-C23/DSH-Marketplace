...
const publicFiles = ['LICENSE', 'DISCLAIMER.md', 'THIRD_PARTY_NOTICES.md', 'scripts/github-secret.ps1', 'licenses/sources/anthropic-apache-2.0.txt', 'catalog/LICENSE-CC0.txt', 'assets/marketplace.png', 'scripts/maintenance.mjs'];
...
export async function buildPackage() {
  ...
  const graph = await build({ entryPoints: [path.join(root, 'src/plugin/index.mjs')], absWorkingDir: root, bundle: true, platform: 'node', format: 'esm', packages: 'external', write: false, metafile: true, logLevel: 'silent' });
  const client = await buildClient();
  const files = new Set([...Object.keys(graph.metafile.inputs), ...publicFiles]);
  for (const entry of await readdir(path.join(root, 'licenses'), { withFileTypes: true })) if (entry.isFile()) files.add(`licenses/${entry.name}`);
  for (const name of await readdir(path.join(root, 'licenses/sources'))) if (name.endsWith('.txt')) files.add(`licenses/sources/${name}`);
  ...
  for (const relative of files) {
    ...
    const packaged = relative === 'scripts/maintenance.mjs' ? 'integration/maintenance.mjs' : relative;
    await mkdir(path.dirname(path.join(stage, packaged)), { recursive: true });
    await copyFile(file, path.join(stage, packaged));
  }
  await writeFile(path.join(stage, 'src/plugin/client.js'), client.code);
  ...
  const notice = (await readFile(path.join(stage, 'THIRD_PARTY_NOTICES.md'), 'utf8')).replace('(package-lock.json)', '(licenses/build-inputs/root-package-lock.json)');
  await writeFile(path.join(stage, 'THIRD_PARTY_NOTICES.md'), notice);
  for (const name of ['README.md', 'README.zh-CN.md', 'README.ja-JP.md']) await copyFile(path.join(root, 'scripts/release', name), path.join(stage, name));
  const plugin = JSON.parse(await readFile(path.join(root, 'src/plugin/package.json'), 'utf8'));
  const manifest = {
    ...
    exports: { '.': './src/plugin/index.mjs', './client': './src/plugin/client.js', './standard-loader': './src/plugin/compatibility/standard-loader.mjs', './package.json': './package.json' },
  ...
  assert(!files.has('src/plugin/package.json'), 'A package must not contain a second frontend identity');
