...
export async function setPlugin(disabled = false, { translationFixture = false, compatibilityFixture = false, resourceFixture = false } = {}) {
  ...
  const plugin = fileURLToPath(new URL('../../src/plugin/index.mjs', import.meta.url)).replaceAll('\\', '/');
  const fixture = fileURLToPath(new URL('../../tests/fixtures/translation-model.mjs', import.meta.url)).replaceAll('\\', '/');
  const compatibility = fileURLToPath(new URL('../../tests/fixtures/compatibility-probe.mjs', import.meta.url)).replaceAll('\\', '/');
  const resource = fileURLToPath(new URL('../../tests/fixtures/resource-probe.mjs', import.meta.url)).replaceAll('\\', '/');
  await writeFile(filename, `# DSH market experiment\n- id: directory-picker\n  disabled: true\n- insert:\n    - id: experiment-directory-backend\n      name: '@deepseek-ai/dsh-host-directory-picker-browse'\n      config:\n        maxEntries: 1000\n    - id: experiment-directory-browser\n      name: '@deepseek-ai/dsh-client-ui-directory-picker-browse'\n    - id: community-market\n      name: '${plugin}'\n      disabled: ${disabled}\n      config:\n        profile: web\n${resourceFixture ? `    - id: resource-test-probe\n      name: '${resource}'\n` : ''}${compatibilityFixture ? `    - id: compatibility-test-probe\n      name: '${compatibility}'\n` : ''}${translationFixture ? `    - id: translation-test-provider\n      name: '${fixture}'\n` : ''}`);
}
