...
import { GitHubCredentials, validateGitHubToken } from '../../src/sources/github-credentials.mjs';
import { createGitHubFetch, GitHubConnection } from '../../src/sources/github-auth.mjs';
...
test('real source and Star defaults share encrypted credentials without authenticating other sources', { skip: process.platform !== 'win32' }, async () => {
  ...
  try {
    ...
    const script = `
      ...
      const { readUpstream, readBytes } = await import(${JSON.stringify(new URL('../../src/sources/request.mjs', import.meta.url).href)});
      const { RepositoryStars } = await import(${JSON.stringify(new URL('../../src/community/repository-stars.mjs', import.meta.url).href)});
