...
import sourceCatalog from '../../catalog/source-seed.json' with { type: 'json' };
import { mergeSourceResources } from '../../src/sources/merge.mjs';
import { SOURCE_DEFINITIONS } from '../../src/sources/definitions.mjs';
import path from 'node:path';
import { lab } from '../../scripts/host/host.mjs';
import { setPlugin } from '../../scripts/host/set-plugin.mjs';
...
test('host state errors stay unknown and retry restores real state', async t => {
  ...
  for (const [locale, settings, field, title] of [
    ...
    const dictionary = JSON.parse(await readFile(`src/languages/${locale}.json`, 'utf8'));
