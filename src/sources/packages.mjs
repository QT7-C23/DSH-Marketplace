...
import { readPinnedSkillBytes, readBytes } from './request.mjs';
import { skillArchivePath } from './skill-bundles.mjs';
...
export async function packageFile(resource, { download = readBytes } = {}) {
  ...
  const archivePaths = bundle.files.map(file => skillArchivePath(bundle, file.path));
  ...
  for (let offset = 0; offset < bundle.files.length; offset += 4) {
    ...
    const results = await Promise.allSettled(batch.map(async file => {
      ...
      return readPinnedSkillBytes(url, file, bundle, download);
    }));
    for (const [index, result] of results.entries()) {
      if (result.status === 'rejected') throw result.reason;
      const file = batch[index];
      files[archivePaths[offset + index]] = [result.value, { mtime: new Date('2000-01-01T00:00:00Z'), os: 3, attrs: (file.mode === '100755' ? 0o100755 : 0o100644) << 16 }];
    }
  }
  files['SOURCE.json'] = [Buffer.from(JSON.stringify({ repository: bundle.repository, commit: bundle.commit, root: bundle.root, files: bundle.files, ...(bundle.licensePaths ? { licensePaths: bundle.licensePaths } : {}) }, null, 2) + '\n'), { mtime: new Date('2000-01-01T00:00:00Z') }];
